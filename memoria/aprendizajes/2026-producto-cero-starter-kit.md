...
- **Registrar el evento en la rama del PR rompe el merge.** Cada run empujaba un commit del orquestador al PR, y los pushes con el token de CI no disparan runs: el head quedaba sin check. Con check requerido, ningún PR habría podido mergearse. Movido a la rama `memoria-eventos` (ADR-0011). (Un run "fallido sin jobs" que al principio atribuí a esto resultó ser el cierre del PR cancelando el run en cola: la primera lectura de una evidencia puede ser errónea; se corrige con rectificación, no borrando.)
